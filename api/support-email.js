...
export default async function handler(req, res) {
  ...
  if (!trimmedMessage || !normalizedEmail || !trimmedPhone) {
    return res.status(400).json({ error: 'Vyplňte správu, email aj telefón.' });
  }
  ...
  const safeUserName = escapeHtml(userName || user.email || 'Používateľ');
  const safeOrgName = escapeHtml(organizationName || 'Neznáma firma');
  ...
  const requestLabel = isFeedback ? 'Podnet na zlepšenie aplikácie' : 'Technická podpora';
  ...
  try {
              ...
              <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:20px;box-shadow:0 18px 45px rgba(15,23,42,.07);overflow:hidden;">
                ...
                <div style="padding:28px 32px;">
                  <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
                    <tr>
                      <td style="padding:8px 0;color:#64748b;font-size:13px;font-weight:700;width:150px;">Typ</td>
                      <td style="padding:8px 0;color:#0f172a;font-size:14px;font-weight:800;">${requestLabel}</td>
                    </tr>
                    <tr>
                      <td style="padding:8px 0;color:#64748b;font-size:13px;font-weight:700;width:150px;">Používateľ</td>
                      <td style="padding:8px 0;color:#0f172a;font-size:14px;font-weight:700;">${safeUserName}</td>
                    </tr>
                    <tr>
                      <td style="padding:8px 0;color:#64748b;font-size:13px;font-weight:700;">Email</td>
                      <td style="padding:8px 0;color:#0f172a;font-size:14px;"><a href="mailto:${safeEmail}" style="color:#ea580c;text-decoration:none;font-weight:700;">${safeEmail}</a></td>
                    </tr>
                    <tr>
                      <td style="padding:8px 0;color:#64748b;font-size:13px;font-weight:700;">Telefón</td>
                      <td style="padding:8px 0;color:#0f172a;font-size:14px;font-weight:700;">${safePhone}</td>
                    </tr>
                    <tr>
                      <td style="padding:8px 0;color:#64748b;font-size:13px;font-weight:700;">ID firmy</td>
                      <td style="padding:8px 0;color:#475569;font-size:13px;font-family:Consolas,monospace;">${safeOrgId}</td>
                    </tr>
                  </table>
                  <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:16px;padding:18px 20px;">
                    <div style="font-size:12px;font-weight:800;color:#9a3412;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">Správa</div>
    ...
    return res.status(500).json({ error: 'Email podpory sa nepodarilo odoslať.' });
  }
}
